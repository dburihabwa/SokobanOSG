...
Sokoban::Board::Board(void): _win(0), _height(0), _width(0), _set(false),_currentLvl(-1),_boxScore(0),_playerScore(0) {
	...
	if((dp  = opendir(LVL_DIR)) == NULL) {
		...
		_message = "Problème lors du chargement des niveaux.";
		notifyViews(WARN_MSG);
		return;
	}
#if DEBUG==TRUE
